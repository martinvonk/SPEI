...
After [@mourik_use_2025] it is possible to visualize the standardized drought indices over several time scales (1, 3, 6, 12, and 24 months) within one graph. This can help with the interpretation of whether or not a drought persists over a long time span. In the case of hydrological drought, there is a relation to the systems response (and recovery) time. The heatmap, as shown in Figure \autoref{fig:spei_heatmap}, indicates such a graph for the SPEI.
...
Meteorological or hydrological data is nowadays typically available on a daily basis. For this reason, the `timescale` argument in for the drought indices' function is designed to be flexible, with its unit matching that of frequency of the the time series. That means that when working with daily data, the user can choose a `timescale` value of 30 for a drought index of 1 month, 90 for 3 months, 180 for 6 months, etc. If the `fit_freq` is daily (`"D"`), 365 (or 366 distributions if the time series has leap years) are fitted, instead of 12 for a monthly `fit_freq` (`"MS"` or `"ME"`). This significantly increases the computational load but can lead to better results as is shown later on.

Dependent on the `fit_freq` and the frequency of the time series, the number of data points per distribution fit can differ. As we saw before in figure \autoref{fig:surplus_fit}, for a time series with monthly observations spanning 30 years and a monthly `fit_freq` this means 30 data points are fitted for each distribution. However, fitting a distribution on only 30 data points, can be challenging. Especially for daily data where the data points are much more prone to outliers. To address this, the `fit_window` argument allows the user to specify a window size (in the frequency of the data) for the fitting procedure. With a `fit_window` of 3 (days), for instance, data from the 14th and 16th of March are also included alongside data from the 15th of March when fitting the distribution. A `fit_window` of 31 days, for daily data, would approximately give the same amount of observations as `fit_freq` equal to a month, with the only difference the number of observations. Which is also implisitly assumed for a monthly `fit_freq`. This feature is still experimental but has been shown to produce more stable results when fitting distributions to daily data. By default, the package determines the `fit_freq` automatically, for frequency to fit the data. If this inference fails, the fallback is a monthly frequency. The `fit_freq` can also be provided manually.

Figure \autoref{fig:surplus_fit_window} shows an example for the different results of the `fit_freq` and `fit_window` on the SPEI-1 as shown for 2001. The top row shows cumulative distributions of precipitation surplus data and corresponding Fisk distribution fits for different settings for the example period in April. The left panel (blue) shows the distribution for a `fit_freq="D"` and no `fit_window` on April 15th only, based on 30 years of daily values. The limited sample size results in a noisier empirical distribution and a less stable fitted distribution. The middle panel (orange) shows a `fit_freq="D"` and a `fit_window=31`, meaning that a distribution on a $\pm$-day window around April 15th, increasing the sample size to 31 days × 30 years and producing a smoother and more robust fit. The right panel (red) shows a monthly fit `fit_freq="MS"` and no fit window, thus using all April data resulting in a robust fit but a less smooth result. This can especially be seen at the beginning of months (e.g. April 1st and November 1st), where large jumps can be observed due to the sharp transition into another distribution. Users can choose themselves which settings they find appropriate for their standardized index.
